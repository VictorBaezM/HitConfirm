...
module.exports = async function handler(req, res) {
  ...
  try {
    const { data: strategy, error: getError } = await supabase
      .from('strategies')
      .select('*')
      .eq('id', strategyId)
      .single();

    if (getError || !strategy) {
      throw new Error(getError?.message || 'Strategy not found');
    }

    const upvotedBy = strategy.upvoted_by || [];
    const index = upvotedBy.indexOf(user.id);
    let upvotes = strategy.upvotes || 0;
    ...
    console.error('Serverless strategy vote failed:', err.message);
