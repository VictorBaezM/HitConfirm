...
async function run() {
  ...
  try {
    ...
    if (json.length > 0) {
      ...
      const anjiMoves = data.filter(m => {
        const char = m.chara || m.character || m.Character || '';
        return char.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() === 'anjimito';
      });
      console.log(`Anji Mito Moves count:`, anjiMoves.length);
      anjiMoves.forEach(m => {
        console.log(`- Name: "${m.name}", Input: "${m.input}"`);
        for (const k of Object.keys(m)) {
          if (m[k] && !['chara', 'character', 'Character', 'images', 'hitboxes', 'name', 'input'].includes(k)) {
            console.log(`  ${k}: "${m[k]}"`);
          }
        }
      });
