...
test.describe('HitConfirm Strategy Hub E2E Tests', () => {
  ...
  test('should display visual moves and ygg-m credit in guides strategy page', async ({ page }) => {
    // 1. Click "Guides" tab in the navbar
    const strategyLink = page.locator('.nav-link[data-page="strategy"]');
    await expect(strategyLink).toBeVisible();
    await strategyLink.click();

    // 2. Verify we are on strategy page
    await expect(page.locator('h1.strategy-title')).toHaveText('STRATEGY HUB');

    // 3. Check that the Matchup Punish table is populated and rendered with visual notations
    const cheatSheetTable = page.locator('.strategy-matrix-table');
    await expect(cheatSheetTable).toBeVisible();

    // Ken's Medium Dragonlash should have a visual move rendering
    const kenMoveCell = cheatSheetTable.locator('.strategy-matrix-td-move:has-text("Medium Dragonlash")');
    await expect(kenMoveCell).toBeVisible();
    const kenSequence = kenMoveCell.locator('.combo-sequence');
    await expect(kenSequence).toBeVisible();
    await expect(kenSequence.locator('.combo-motion')).toHaveText('DP');
    await expect(kenSequence.locator('.combo-btn')).toHaveText('MK');

    // Ky's Stun Dipper should have visual move rendering
    const kyMoveCell = cheatSheetTable.locator('.strategy-matrix-td-move:has-text("Stun Dipper")');
    await expect(kyMoveCell).toBeVisible();
    const kySequence = kyMoveCell.locator('.combo-sequence');
    await expect(kySequence).toBeVisible();
    await expect(kySequence.locator('.combo-motion')).toHaveText('QCF');
    await expect(kySequence.locator('.combo-btn')).toHaveText('K');

    // Yoshimitsu's Flash (d+1+4)
    const yoshiMoveCell = cheatSheetTable.locator('.strategy-matrix-td-move:has-text("Flash")');
    await expect(yoshiMoveCell).toBeVisible();
    const yoshiSequence = yoshiMoveCell.locator('.combo-sequence');
    await expect(yoshiSequence).toBeVisible();
    await expect(yoshiSequence.locator('.combo-dir')).toHaveText('↓');
    await expect(yoshiSequence.locator('.combo-btn').first()).toHaveText('1');
    await expect(yoshiSequence.locator('.combo-btn').last()).toHaveText('4');

    // 4. Verify that the credit note is displayed and contains the Fighting Game Input Translator link
    const creditNote = page.locator('.strategy-credit-note');
    await expect(creditNote).toBeVisible();
    await expect(creditNote).toContainText('Notation translations powered by');
    const creditLink = creditNote.locator('a[href="https://ygg-m.github.io/fg-input-translator/"]');
